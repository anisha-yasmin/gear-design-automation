...
st.set_page_config(page_title="Gear Synthesis Suite", layout="wide")

st.title("⚙️ Parametric Gear Synthesis & Optimization Suite")
st.markdown("Automated AGMA structural optimization and full-profile geometric generation.")
st.markdown("---")

# Layout columns for inputs and outputs
...
with col_right:
    if execute:
        ...
        if result:
            ...
            m_col1.metric("Module", f"{result['Module']} mm")
            m_col2.metric("Pinion Teeth (N1)", int(result['Pinion Teeth (N1)']))
            m_col3.metric("Face Width", f"{result['Face Width (mm)']} mm")
            ...
            # Get base profile coordinates for one tooth face
            x_face, y_face = generate_involute_points(rb, ra)
            
            fig = go.Figure()
            
            # Draw standard reference circles
            ...
            # Generate and rotate teeth profiles around the full 360 degrees
            for i in range(N):
                beta = (2 * np.pi * i) / N  # Rotation angle for each tooth position
                
                # Direct coordinate transformation matrix rotation
        ...
        st.info("Awaiting input initialization. Adjust constraints on the left sidebar and execute the loop.")
